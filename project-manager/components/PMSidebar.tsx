...
export default function PMSidebar({ openNotifications = () => {} }: SidebarProps) {
  ...
  const menuItems = [
    ...
    { id: 'calendar', label: t('pmDashboard.sidebar.meetingCalendar'), icon: Calendar },
    { id: 'messages', label: t('pmDashboard.sidebar.secureMessaging'), icon: MessageSquare, badge: hasUnreadChat },
    { id: 'reports', label: t('pmDashboard.sidebar.reportsAnalytics'), icon: BarChart },
    { id: 'requests', label: t('pmDashboard.sidebar.consultantRequests'), icon: Users },
  ];
    ...
    <div className={`flex flex-col h-full bg-surface border-r border-white/10 p-4 lg:p-6 z-40 select-none overflow-y-auto transition-all duration-300`}>
      ...
      <div className={`pt-6 border-t border-white/5 ${isCollapsedView ? 'space-y-4 flex flex-col items-center' : 'space-y-4'}`}>
        ...
        >
          ...
          {!isCollapsedView && (
            <div className="min-w-0 flex-1">
              <h3 className="text-sm font-extrabold text-white truncate hover:underline">{t('pmDashboard.sidebar.projectManager')}</h3>
              <p className="text-[10px] text-slate-400 truncate capitalize">
                {t('pmDashboard.sidebar.seniorOperations')}
              </p>
            </div>
          )}
        </Link>
        ...
        <button
          ...
          title={isCollapsedView ? 'Notifications' : undefined}
        ...
        >
          <div className="flex items-center gap-3">
            ...
            {!isCollapsedView && <span>{t('pmDashboard.sidebar.notifications')}</span>}
          </div>
        ...
        <button
          ...
          title={isCollapsedView ? t('pmDashboard.sidebar.signOut') : undefined}
        ...
        >
          ...
          {!isCollapsedView && <span>{t('pmDashboard.sidebar.signOut')}</span>}
        </button>
      </div>
    </div>
  );
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="max-w-sm w-full bg-card border border-red-500/20 backdrop-blur-2xl p-6 lg:p-8 rounded-[2rem] space-y-6 shadow-2xl relative">
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <h3 className="text-xl font-extrabold text-white">{t('pmDashboard.sidebar.signOut')}</h3>
              <p className="text-sm text-slate-400">{t('pmDashboard.sidebar.confirmSignOut')}</p>
            </div>
